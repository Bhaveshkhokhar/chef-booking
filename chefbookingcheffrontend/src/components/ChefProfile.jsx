...
const ChefProfile = () => {
  const {chef,updateProfilePic}=useContext(ChefStore);
    ...
    const [previewImg, setPreviewImg] = useState(
      `http://localhost:3001${chef.image}`
    );
    useEffect(() => {
      setPreviewImg(`http://localhost:3001${chef.image}`);
    }, [chef.image]);
    ...
    const handleUpload = () => {
      if (selectedFile) {
        ...
        fetch("http://localhost:3001/updateChefProfilePic", {
          method: "POST",
          body: formData,
          credentials: "include",
        })
    ...
    const handleCancel = () => {
      setShowUploadPopup(false);
      setPreviewImg(`http://localhost:3001${chef.image}`); // Reset preview on cancel
    };
